...
public sealed class LoggingMiddleware : ITrapMiddleware
{
    ...
    {
        _logger.LogDebug(
            "Processing trap from {SenderAddress}, correlationId: {CorrelationId}, varbinds: {VarbindCount}",
        ...
        {
            _logger.LogDebug(
                "Trap rejected: {RejectionReason}, sender: {SenderAddress}, correlationId: {CorrelationId}",
        ...
        {
            _logger.LogDebug(
                "Trap routed to device {DeviceName}, definition: {MetricName}, correlationId: {CorrelationId}",
                context.Device.Name,
